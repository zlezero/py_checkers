...
## Fonctions utilisée lorsque l'on clique 
    ...
    def showPlaceToGo(self, pionSelect): #Montre les endroits possibles sur le damier
        ...
        if self.TableauDames[pionSelect].Status == "Dame":
            numberToMutiply = 2
            for i in listeCheck:
                while numberToMultipy < 8:
                    self.CercleChoixPossible.append(self.canvas.create_oval(self.TableauDames[pionSelect + (i * numberToMultiply)].PosX - 5, self.TableauDames[pionSelect + (i * numberToMultiply)].PosY - 5, self.TableauDames[pionSelect + (i * numberToMultiply)].PosX + 5, self.TableauDames[pionSelect + (i * numberToMultiply)].PosY + 5, fill= "yellow"))
                        
                    self.TableauCaseChoixPossible.append(pionSelect + (i * numberToMultiply))
