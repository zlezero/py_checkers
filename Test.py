...
class MainMenu(): #Classe représentant le menu principal
    ...
    def Open_GameWindow1V1(self): #Fonction ouvrant la fenêtre de jeu
        ...
        self.app = Jeu(self.newWindow, 1, False)
...
class Multijoueur(): #Classe représentant le menu des options
    ...
    def Draw_Interface(self):
        ...
        self.Button_Launch = Button(self.Frame, text = "Lancer", command = self.Launch_Multi)
    ...
    def Launch_Multi(self):
        ...
        if self.isHost == 0 and self.Ip != "":
            print("Connecting !")
            self.Network = Network(self.Ip, self.isHost)
            self.Connect()
        elif self.isHost == 1:
            print("Hosting !")
    ...
    def Connect(self):
        print("...")
        self.Client = Thread(target = self.Network.client(False))
        self.Serveur = Thread(target = self.Network.serveur(True))

    def Host(self):
        print("...")
        self.Client = Thread(target = self.Network.serveur(False))
        self.Serveur = Thread(target = self.Network.client(True))
...
class Network():
    ...
    def serveur(self, secondInstance):


        if secondInstance == False:
            port = self.port
        else:
            port = self.port + 1
    ...
    def client(self, secondInstance):    
...
class Jeu(): #Classe représentant l'interface du jeu de dames
    ...
    def __init__(self, master, nbrJoueurs, isMultiplayer): #Initialisation de l'interface et de la classe
        ...
        self.GEng = GameEngine(self.can, isMultiplayer)
...
class GameEngine():
    ...
    def __init__(self, canvas, isMultiplayer):
...
## Fonctions utilisée lorsque l'on clique 
    ...
    def showPlaceToGo(self, pionSelect): #Montre les endroits possibles sur le damier
        ...
        if self.priseMultiple != True or canTakePion == True:
